...
class Trainer(object):
    ...
    def train_epoch_gen(self, model, epoch_idx):
    ...
    def train_epoch_dis(self, model, epoch_idx):
    ...
    def train(self, model):
        ...
        for epoch_idx in range(train_config['epoch']):
            ...
            if configs['model']['type'] == 'discriminative':
                self.train_epoch_dis(model, epoch_idx)
            else:
                self.train_epoch_gen(model, epoch_idx)
            end_time = time.time()
        ...
        test_file = './save/result/' + configs['model']['name'] + '_' + configs['data']['name'] + '.txt'
        with open(test_file, 'a') as f:
            f.write("Best Epoch {}. Final test result: {}. \n".format(best_epoch, test_result))
    ...
    def save_model(self, model):
        if configs['train']['save_model']:
            ...
            if not configs['tune']['enable']:
                save_dir_path = './save/checkpoint/{}'.format(model_name)
                ...
                save_dir_path = './save/checkpoint/{}/tune'.format(model_name)
